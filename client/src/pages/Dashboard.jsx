import React, { useEffect, useState } from 'react'
import { dummyCreationData } from '../assets/assets'
import { GemIcon, Sparkles } from 'lucide-react'
import { Protect } from '@clerk/clerk-react'
import CreationItem from '../components/CreationItem'

const Dashboard = () => {
  const [creations, setCreations] = useState([])

  const getDashboardData = async () => {
    setCreations(dummyCreationData)
  }

  useEffect(() => {
    getDashboardData()
  }, [])

  return (
    <div className="h-full overflow-y-scroll p-6">
      <div className="flex justify-start gap-4 flex-wrap">
        {/* Total Creations Card */}
        <div className="flex justify-between items-center w-72 p-6 bg-white rounded-xl border border-gray-200">
          <div>
            <p className="text-slate-600">Total Creations</p>
            <h2 className="text-xl font-semibold">{creations.length}</h2>
          </div>
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-[#3585F8] to-[#8FBEF5] text-white flex justify-center items-center">
            <Sparkles className="w-5 text-white" />
          </div>
        </div>

        {/* Total Active Card */}
        <div className="flex justify-between items-center w-72 p-6 bg-white rounded-xl border border-gray-200">
          <div>
            <p className="text-slate-600">Total Active</p>
            <h2 className="text-xl font-semibold">
              <Protect plan='Premium' fallback="Free " >Premium</Protect>
            </h2>
          </div>
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-[#FF6A5B] to-[#FFC371] text-white flex justify-center items-center">
            <GemIcon className="w-5 text-white" />
          </div>
        </div>

      </div>


      <div className='space-y-3'>
        <p className='text-lg font-semibold mt-6'>
          Recent Creation
        </p>
        {
          creations.map((item) => <CreationItem key={item.id} item={item} />)
        }

      </div>
    </div>
  )
}

export default Dashboard